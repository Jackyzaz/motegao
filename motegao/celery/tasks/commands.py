...
def run_command_subdomain_enum(domain: str, threads: int = 10, wordlist: int = 1):
    ...
    for progress in run_command_subdomain_enum_yielder(
        ...
        run_command_subdomain_enum.backend.mark_as_started(
            run_command_subdomain_enum.request.id, subdomains=results
        )
...
def run_command_subdomain_enum_yielder(cmd):
    ...
    p = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE, bufsize=1, text=True)
